# AAIF Wallet Authorization Protocol (WAuth) v0.4.0 — Wallet Authorization over MCP (Draft)
...
- **WAUTH-POLICY-MGMT** — Policy authoring, distribution, and DSL guidance (informative)
...
### 15.3 WAUTH-POLICY-MGMT — Policy management and DSL guidance (informative)

Large-scale adoption requires a consistent way to author, review, version, and deploy policy across:
...
This specification intentionally does not mandate one policy engine. Instead it standardizes **policy inputs and outputs** and provides suggested DSL profiles.
...
- `permit`
- `deny`
- `wauth_required` + `authorization_details` (requirements template) and optionally `assurance` constraints

Suggested DSL/profile options (informative):
...
Conformance requirements are defined in `conformance-v0.4.0.md`.
