# AAIF Wallet Authorization Protocol (WAuth) v0.5.1 — Wallet Authorization over MCP (Draft)
...
Version **0.5.1** clarifies the boundary between the core protocol and optional profiles, and adds operational-safety profiles for requester identity continuity, instruction-source integrity, execution budgets, postcondition verification, and multi-agent trust.
...
- Treat **privileged or side-effecting authority** as something that must be grounded in verifiable requester identity, explicit approval, policy-approved automation, or an already-valid mandate/capability — never in conversational cues alone.

A conforming implementation MAY additionally claim one or more optional profiles (Section 15). If a deployment claims a profile, it MUST implement that profile’s normative requirements.
...
- **Requester (RQ):** the human or service asking an Agent Host to act. For privileged or side-effecting actions, requester identity and requester authorization MAY be policy-relevant and MUST NOT be inferred solely from display names, tone, avatars, or channel-local cues.
...
- **Profile-defined artifacts:** optional artifacts such as `WAUTH-REQUESTER`, `WAUTH-INSTR`, `WAUTH-BUDGET`, `WAUTH-POST`, and `WAUTH-AGENT-LINK` MAY be used by claimed profiles without changing the core mandate/capability model.
...
### 14.5 Requester authority boundary (MUST for privileged or side-effecting actions)

For any privileged or side-effecting action whose legitimacy depends in whole or in part on **who requested it**, a conforming deployment MUST treat requester identity and requester authorization as a distinct security boundary.

Specifically:

- A deployment MUST NOT treat conversational cues alone as sufficient authority.
- Display names, nicknames, avatars, tone, channel-local handles, prior familiarity, urgency, or politeness MUST be treated as **non-authoritative cues**.
- If channel or session context changes in a way that breaks requester identity continuity, privileged trust MUST be re-established before execution.
- A stable requester identifier, verifier-backed requester assertion, or fresh step-up approval MUST be required whenever policy says requester identity is material to the action.

The optional **WAUTH-REQUESTER-CONTINUITY** profile (Section 15.8) defines interoperable artifacts and rules for satisfying this requirement.
...
- **WAUTH-REQUESTER-CONTINUITY** — Requester identity continuity and requester authorization for privileged actions (normative)
- **WAUTH-INSTRUCTION-SOURCE-INTEGRITY** — Trust classes and promotion rules for instruction-bearing sources (normative)
- **WAUTH-EXEC-BUDGETS** — Execution budgets and persistent side-effect controls (normative)
- **WAUTH-POSTCONDITION** — Verified outcome checks and tool/RP receipts (normative)
- **WAUTH-MULTI-AGENT-TRUST** — Multi-agent delegation, provenance, and anti-amplification rules (normative)
...
**Profile identifier:**
- `aaif.wauth.profile.policy-management/v0.2`

#### 15.3.1 Normative policy interface

A WAS or RP claiming this profile MUST support:

- **Policy Context schema** (input): `schemas/wauth-policy-context.v0.2.schema.json`
...
- requester facts (`requester_id`, `requester_roles`, `continuity_method`)
- instruction-source facts (`authority_class`, `mutability`, `loaded_as`, `review_state`)
- execution-budget facts (`process_max`, `storage_bytes_max`, `may_create_persistent_process`, `ttl_seconds`)
- postcondition expectations (`expected_postcondition`)
- multi-agent facts (`relation`, `assertion_basis`, `verified_independently`)
...
### 15.8 WAUTH-REQUESTER-CONTINUITY — Requester identity continuity and requester authorization (normative)

**Problem:** Deployed agents often authenticate themselves strongly to tools while treating the *human requester* weakly. Owner spoofing across channels, display-name confusion, and “tone-based” trust show that **who is allowed to command the agent** is itself a security boundary.

**Profile identifier:**
- `aaif.wauth.profile.requester-continuity/v0.1`

#### 15.8.1 Normative object

Implementations claiming this profile MUST support:

- `schemas/wauth-requester-context.v0.1.schema.json`

The object captures, at minimum:

- a stable `requester_id` or equivalent verifier-backed identity reference,
- `requester_type` and `requester_roles`,
- the channel/session context in which the requester appeared,
- the method used to establish continuity across channels or sessions, and
- a list of cues that are explicitly non-authoritative.

#### 15.8.2 Non-authoritative cues (MUST)

Display names, nicknames, avatars, channel-local handles, writing style, tone, emotional pressure, urgency, or prior conversational familiarity MUST NOT be treated as sufficient authority for privileged or side-effecting actions.

#### 15.8.3 Channel and session continuity (MUST)

If a requester initiates a privileged action in a new channel or session, and continuity to a previously trusted requester cannot be established using a stable identifier or other verifier-backed method, the system MUST treat the requester as unverified for privileged purposes until continuity is re-established or step-up occurs.

#### 15.8.4 Requester authorization (MUST)

This profile distinguishes **who the requester is** from **what the requester may ask the agent to do**. Implementations MUST support requester roles or equivalent policy facts (for example `owner`, `operator`, `delegate`, `maintainer`) so privileged requests can be denied even when requester identity is known but requester authority is insufficient.

### 15.9 WAUTH-INSTRUCTION-SOURCE-INTEGRITY — Instruction source integrity and mutable memory controls (normative)

**Problem:** Agents frequently treat mutable memory files, wiki pages, gists, tool outputs, and other externally editable resources as if they were authoritative instructions. This collapses the boundary between **data** and **policy**, and makes indirect prompt injection durable across sessions.

**Profile identifier:**
- `aaif.wauth.profile.instruction-source-integrity/v0.1`

#### 15.9.1 Normative object

Implementations claiming this profile MUST support:

- `schemas/wauth-instruction-source.v0.1.schema.json`

Each source object classifies a source by:

- `authority_class` (`authoritative`, `reference`, `data`, `untrusted`),
- `mutability` (`immutable`, `versioned`, `append_only`, `mutable`, `externally_editable`),
- `loaded_as` (`instruction`, `policy`, `memory`, `reference`, `data`), and
- integrity/review facts such as signature, digest, reviewer state, and provenance reference.

#### 15.9.2 Default handling rules (MUST)

- Sources that are externally editable or policy-unreviewed MUST default to `authority_class = data` or `untrusted`.
- Such sources MUST NOT be treated as authoritative instructions merely because they are stored in memory, linked from memory, or repeatedly cited in conversation.
- Promotion from `data` or `reference` into `instruction` or `policy` MUST require an explicit policy-approved transition and SHOULD emit a provenance event.

#### 15.9.3 Authoritative source requirements (SHOULD)

Deployments SHOULD require authoritative instruction sources to be versioned, integrity-protected, and reviewable. Signed policy bundles, immutable repository blobs, or enterprise-controlled configuration channels are RECOMMENDED over mutable shared documents.

### 15.10 WAUTH-EXEC-BUDGETS — Execution budgets and persistent side-effect controls (normative)

**Problem:** Short conversational requests can create long-lived or unbounded infrastructure changes: infinite loops, cron jobs, background processes, storage growth, and other persistent side effects.

**Profile identifier:**
- `aaif.wauth.profile.execution-budgets/v0.1`

#### 15.10.1 Normative object

Implementations claiming this profile MUST support:

- `schemas/wauth-exec-budget.v0.1.schema.json`

The budget object MAY constrain:

- maximum runtime or TTL,
- model tokens or turns,
- process count,
- storage growth,
- network requests/bytes, and
- whether persistent processes or recurring schedules may be created.

#### 15.10.2 Persistent side effects (MUST)

No persistent side effect MAY be created unless authority for that class of side effect was explicitly granted by policy or by a satisfying mandate/capability. Background jobs, cron tasks, daemons, monitors, or recurring workflows MUST require an explicit allow flag (for example `may_create_persistent_process = true`) and SHOULD require bounded termination conditions.

#### 15.10.3 Budget-aware fail-safe behavior (MUST)

If required budget fields are missing, stale, or would be exceeded by the requested operation, the system MUST fail closed by returning `deny` or `wauth_required`.

### 15.11 WAUTH-POSTCONDITION — Verified outcomes and tool/RP receipts (normative)

**Problem:** Agents often narrate success without proving that the underlying state transition actually happened, or they leave unsafe side effects in place after reporting completion.

**Profile identifier:**
- `aaif.wauth.profile.postcondition-verification/v0.1`

#### 15.11.1 Normative object

Implementations claiming this profile MUST support:

- `schemas/wauth-postcondition-receipt.v0.1.schema.json`

The receipt distinguishes at least:

- the action that was authorized,
- whether the agent *claimed* success,
- whether an RP, tool, or secondary verifier *verified* success,
- the observed state hash or evidence reference, and
- which postconditions passed, failed, or remain unknown.

#### 15.11.2 Required behavior (MUST)

For destructive, stateful, or otherwise high-impact actions, implementations MUST distinguish “claimed success” from “verified outcome”. If postcondition verification fails or is unavailable when required by policy, the action MUST be treated as incomplete or failed for purposes of audit and downstream automation.

### 15.12 WAUTH-MULTI-AGENT-TRUST — Multi-agent delegation, provenance, and anti-amplification (normative)

**Problem:** In multi-agent systems, vulnerabilities propagate across agent hops, circular corroboration creates false confidence, and responsibility becomes diffuse.

**Profile identifier:**
- `aaif.wauth.profile.multi-agent-trust/v0.1`

#### 15.12.1 Normative object

Implementations claiming this profile MUST support:

- `schemas/wauth-agent-link.v0.1.schema.json`

The object records a trust-bearing relationship between agents, including:

- parent/child or peer identities,
- the asserted relation (`advisor`, `tool`, `delegator`, `executor`, `peer`, etc.),
- the basis of the assertion,
- whether independent verification occurred, and
- whether transitive trust is prohibited.

#### 15.12.2 Anti-amplification rules (MUST)

- Assertions from one agent about another MUST NOT become trusted solely because multiple agents repeat them.
- When Agent A causes Agent B to act, the provenance chain and delegated authority chain MUST remain visible and attributable across the hop.
- Shared-channel observations MUST NOT be treated as proof of identity continuity or independent corroboration without an external anchor.

#### 15.12.3 Budget interaction (SHOULD)

Multi-agent conversations, relays, and monitoring patterns SHOULD be governed by explicit execution budgets (Section 15.10) to prevent unbounded loops and silently persistent coordination tasks.

### 15.13 WAUTH-AAUTH-ACQ — AAuth acquisition compatibility (informative)
...
Conformance requirements are defined in `conformance-v0.5.1.md`.
