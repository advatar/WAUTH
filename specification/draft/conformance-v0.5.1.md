# AAIF Wallet Authorization Protocol (WAuth) — Conformance Requirements v0.5.1 (Draft)
...
- **WAS-REQUESTER** — requester identity continuity and requester authorization support.
- **WAS-INSTRUCTION** — instruction-source integrity classification and promotion controls.
- **WAS-BUDGETS** — execution budgets and persistent side-effect controls.
- **WAS-POSTCOND** — postcondition verification receipts and verified-outcome handling.
- **WAS-MULTI-AGENT** — multi-agent link/provenance handling and anti-amplification controls.
...
- **RP-REQUESTER** — requester identity continuity requirements for privileged requests.
- **RP-POSTCOND** — requires and verifies postcondition receipts when policy demands them.
- **RP-MULTI-AGENT** — enforces multi-agent provenance and anti-amplification rules.
...
- **MCP-SAFETY** — forwards requester context, instruction-source descriptors, execution budgets, postcondition expectations, and agent-link facts when policy or profiles require them.
...
**WAS-POL-01 (MUST):** Accept policy context objects conforming to `schemas/wauth-policy-context.v0.2.schema.json`.
...
### 2.13 Requester continuity (WAS-REQUESTER)

**WAS-REQ-01 (MUST):** Support `schemas/wauth-requester-context.v0.1.schema.json`.

**WAS-REQ-02 (MUST):** Treat display names, nicknames, avatars, tone, or channel-local cues as non-authoritative for privileged actions.

**WAS-REQ-03 (SHOULD):** Require continuity re-establishment or step-up when requester context changes materially across channels or sessions.

### 2.14 Instruction-source integrity (WAS-INSTRUCTION)

**WAS-INS-01 (MUST):** Support `schemas/wauth-instruction-source.v0.1.schema.json`.

**WAS-INS-02 (MUST):** Default externally editable or unreviewed sources to non-authoritative classes unless explicitly promoted by policy.

### 2.15 Execution budgets (WAS-BUDGETS)

**WAS-BUD-01 (MUST):** Support `schemas/wauth-exec-budget.v0.1.schema.json`.

**WAS-BUD-02 (MUST):** Refuse persistent side effects unless explicitly authorized by policy or capability.

### 2.16 Postcondition verification (WAS-POSTCOND)

**WAS-POST-01 (MUST):** Support `schemas/wauth-postcondition-receipt.v0.1.schema.json`.

**WAS-POST-02 (SHOULD):** Emit receipts that distinguish claimed success from verified outcome.

### 2.17 Multi-agent trust (WAS-MULTI-AGENT)

**WAS-MAG-01 (MUST):** Support `schemas/wauth-agent-link.v0.1.schema.json`.

**WAS-MAG-02 (MUST):** Preserve delegation and provenance across agent hops when a claimed profile uses multi-agent execution.

### 3.2 MCP safety forwarding (MCP-SAFETY)

**MCP-SAFE-01 (SHOULD):** Forward requester context, instruction-source descriptors, execution budgets, expected postconditions, and agent-link facts when the WAS or RP requires them.

### 4.8 Requester continuity (RP-REQUESTER)

If an RP claims **RP-REQUESTER**, it MUST implement the WAUTH-REQUESTER-CONTINUITY profile, including:

**RP-REQID-01 (MUST):** Refuse privileged requests whose authority depends on requester identity when continuity cannot be established using a stable identifier or fresh step-up.

**RP-REQID-02 (MUST):** Treat display names, tone, or other conversational cues as non-authoritative.

### 4.9 Postcondition requirements (RP-POSTCOND)

If an RP claims **RP-POSTCOND**, it MUST implement the WAUTH-POSTCONDITION profile, including:

**RP-POST-01 (MUST):** Require or emit postcondition receipts for operations designated by policy as destructive, stateful, or otherwise high-impact.

**RP-POST-02 (SHOULD):** Distinguish claimed success from verified outcome in audit and downstream decision logic.

### 4.10 Multi-agent trust (RP-MULTI-AGENT)

If an RP claims **RP-MULTI-AGENT**, it MUST implement the WAUTH-MULTI-AGENT-TRUST profile, including:

**RP-MAG-01 (MUST):** Reject transitive or circular corroboration as sufficient evidence of identity or authority without an independent anchor.

**RP-MAG-02 (SHOULD):** Require provenance across agent hops when delegated execution traverses multiple agents.
